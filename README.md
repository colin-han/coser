...
根据当前 WiFi 网络、账户余额等条件自动选择合适的 profile 启动 Claude Code。

## 功能

- **自动切换** — 根据连接的 WiFi SSID 自动选择 profile，余额不足时自动 fallback
...
```bash
mkdir -p ~/.coser/profiles
```

### 2. 创建 Profile

在 `~/.coser/profiles/` 下创建 TOML 文件，例如 `glm.toml`：
